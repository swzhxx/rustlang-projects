...
use rm::learning::k_means::{Forgy, KMeansClassifier, KPlusPlus, RandomPartition};
...
impl Flower {
    ...
    fn into_labels(&self) -> u64 {
        match self.species.as_str() {
            ...
            "virginica" => 2,
            l => panic!(
                "Not able to parse the label. Some other label got passed. {:?}",
                l
            ),
        }
    }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let flower_x_train: Vec<f64> = train_data
        .iter()
    ...
    let flower_x_test: Vec<f64> = test_data
        .iter()
        .flat_map(|r| r.into_feature_vector())
        .collect();
    ...
    println!(
        "number of Probablities from GMM: {:?}",
        classes.data().len()
    );
